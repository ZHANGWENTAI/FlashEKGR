...
def parse_args(args=None):  # pylint: disable=unused-argument
    ...
    parser.add_argument(
        "--lr_schedule", default="none", type=str, choices=["none", "step"], help="learning rate scheduler"
    )
    parser.add_argument("--kg_dtype", default="uint32", type=str, choices=["uint32", "uint64"], help="data type of kg")

    parser.add_argument("--print_on_screen", action="store_true")

    tasks_default = "1p.2p.3p.2i.3i.ip.pi.2in.3in.inp.pin.pni.2u.up"
    parser.add_argument(
        "--tasks",
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="training tasks connected by dot, refer to the BetaE paper "
        "for detailed meaning and structure of each task",
    )
    ...
    parser.add_argument(
        "-rotatem", "--rotate_mode", default="(Mean,True)", type=str, help="(intersection aggr,nonlinearity) for Rotate"
    )
    parser.add_argument(
        "-complexm",
        "--complex_mode",
        default="(Mean,True)",
        type=str,
        help="(intersection aggr,nonlinearity) for Complex",
    )
    ...
    parser.add_argument(
        ...
        default="(Mean,True)",
        type=str,
        help="(intersection aggr,nonlinearity) for DistMult",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        choices=["DNF", "DM"],
        help="the way to evaluate union queries, transform it to "
        "disjunctive normal form (DNF) or use the De Morgan's laws (DM)",
    )
    ...
    online_sample_mode_default = "(500,-1,w,wstruct,80,True,True)"
    parser.add_argument(
        "--online_sample_mode",
        default=online_sample_mode_default,
        type=str,
        help="(0,0,w/u,wstruct/u,0,True,True) or "
        "(relation_bandwidth,max_num_of_intermediate,w/u,wstruct/u,"
        "max_num_of_partial_answer,weighted_ans,weighted_neg)",
    )
    parser.add_argument(
        "--online_weighted_structure_prob", default="(70331,141131,438875)", type=str, help="(same,0,w/u,wstruct/u)"
    )

    parser.add_argument("--gpus", default="-1", type=str, help="gpus")
    parser.add_argument(
        "--logit_impl", default="native", type=str, help="logit implementation", choices=["native", "custom"]
    )
    parser.add_argument("--port", default="29500", type=str, help="dist port")
    train_online_mode_default = "(single,500,er,False,before)"
    parser.add_argument(
        "--train_online_mode",
        default=train_online_mode_default,
        type=str,
        help="(mix/single,sync_steps,er/e/r/n trained on cpu," "async flag, before/after)",
    )
    parser.add_argument(
        "--optim_mode",
        default="(fast,adagrad,cpu,True,5)",
        type=str,
        help="(fast/aggr,adagrad/rmsprop,cpu/gpu,True/False,queue_size)",
    )

    parser.add_argument("--reg_coeff", default=0.0, type=float, help="margin in the loss")
    parser.add_argument(
        "--neg_sample_size_eval", type=int, default=0, help="neg sample size for eval; set 0 to eval against all"
    )
