...
from flashekgr.common.config import all_tasks, name_query_dict, parse_args, query_name_dict
from flashekgr.common.embedding.embed_optimizer import get_optim_class
from flashekgr.common.util import eval_tuple, maybe_download_dataset, parse_time, set_global_seed
from flashekgr.cpp_sampler.sampler_clib import KGMem
from flashekgr.evaluation.dataloader import MultihopTestDataset, Test1pBatchDataset, Test1pDataset
from flashekgr.models import build_model
from flashekgr.training.train_process import async_aggr, train_mp

QueryData = namedtuple("QueryData", ["data", "buffer", "writer_buffer"])


def setup_train_mode(args):
    """Setup training mode configuration."""
    ...
    if args.online_sample:
        if eval_tuple(args.online_sample_mode)[3] == "wstruct":
            prob_array = np.array(eval_tuple(args.online_weighted_structure_prob)).astype(np.float32)
            normalized_structure_prob = (prob_array / np.sum(prob_array)).tolist()
        ...
        update_mode, optimizer_name, optimizer_device, _, _ = eval_tuple(args.optim_mode)
...
def setup_save_path(args):
    ...
    args.save_path = os.path.join(prefix, data_name, f"{args.training_tasks}-{args.tasks}", args.geo)
...
def get_model(args):
    ...
    stats_path = os.path.join(args.data_path, "stats.txt")
...
def load_1p_eval_data(args, phase):
    ...
    if "head_neg" in all_data:  # bi-directional
        ...
        fwd_data = {"head": all_data["head"], "relation": all_data["relation"] * 2, "tail": all_data["tail"]}
        if "tail_neg" in all_data:
            fwd_data["tail_neg"] = all_data["tail_neg"]
        backwd_data = {"head": all_data["tail"], "relation": all_data["relation"] * 2 + 1, "tail": all_data["head"]}
        ...
        fwd_data = {"head": all_data["head"], "relation": all_data["relation"], "tail": all_data["tail"]}
        if "tail_neg" in all_data:
            fwd_data["tail_neg"] = all_data["tail_neg"]
...
def write_to_writer(eval_dict, writer):
    """Write evaluation metrics to tensorboard writer."""

    def collect_and_write(writer_buffer, mode):
        metrics, average_metrics, step = writer_buffer.get()
        if step == -1:
            return False
        for query_structure, struct_metrics in metrics.items():
            for metric, value in struct_metrics.items():
                ...
                writer.add_scalar("_".join([mode, qname, metric]), value, step)
        for metric, value in average_metrics.items():
            writer.add_scalar("_".join([mode, "average", metric]), value, step)
        return True

    writer_flag = True
    while writer_flag:
        writer_flag = False
        for key, query_data in eval_dict.items():
            if collect_and_write(query_data.writer_buffer, key):
                writer_flag = True
...
def main(parser):
    ...
    expected_gpus = ".".join([str(i) for i in range(len(gpus))])
    assert args.gpus == expected_gpus, (
    ...
    if args.do_train:
        ...
        logging.info("%s: infinite", args.training_tasks)
    ...
    for phase in ["valid", "test"]:
        if getattr(args, f"do_{phase}", False):
    ...
    if args.feature_folder is not None:
        ...
        entity_feat_path = os.path.join(args.feature_folder, "entity_feat.npy")
        relation_feat_path = os.path.join(args.feature_folder, "relation_feat.npy")
        ro_feat = {
            "entity": torch.tensor(np.load(entity_feat_path), dtype=torch.float16),
            "relation": torch.tensor(np.load(relation_feat_path), dtype=torch.float16),
        }
    else:
        ro_feat = None
    procs = []
    ...
    for rank, gpu_id in enumerate(gpus):
        logging.info("[GPU %d] tasks: %s", gpu_id, args.training_tasks)
        local_eval_dict = {}
        for phase, q_data in eval_dict.items():
            nq_per_proc = math.ceil(len(q_data.data) / len(gpus))
            local_eval_dict[phase] = QueryData(
                q_data.data.subset(rank * nq_per_proc, nq_per_proc), q_data.buffer, q_data.writer_buffer
            )
        proc = mp.Process(
            target=train_mp, args=(args, kg_mem, opt_stats, model, local_eval_dict, training_tasks, ro_feat, gpu_id)
        )
        procs.append(proc)
        proc.start()
