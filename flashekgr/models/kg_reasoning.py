...
from flashekgr.common.embedding.sparse_embed import SparseEmbedding
from flashekgr.common.embedding.embed_rw import EmbeddingReadOnly
from flashekgr.common.util import cal_ent_loc_dict
from flashekgr.common.cuda_graph import enable_cuda_graph

from flashekgr.models.query_parser import OpType
...
class KGReasoning(nn.Module):
    def __init__(
        ...
        self.batch_entity_range = (
            ...
            if self.use_cuda
            else torch.arange(nentity).to(torch.float).repeat(test_batch_size, 1)
        )  # used in test_step
    ...
    def merge_embeddings(self, embedding_list, dim, expand_dim=None):
        ...
        merged_embedding_list = [
            (
                torch.cat(embedding, dim=dim).unsqueeze(dim=expand_dim)
                if expand_dim is not None
                else torch.cat(embedding, dim=dim)
            )
            for embedding in merged_embedding_list
        ]
    ...
    def is_all_relation(self, query_structure):
        ...
        # whether the current query tree has merged to one branch and only need to
        # do relation traversal, e.g., path queries or conjunctive queries after
        # the intersection
    ...
    def forward(
        self, positive_sample, negative_sample, query_structure, query_matrix, device, all_neg=None, reg_coeff=0.0
    ):
        # #!
        ...
        if negative_sample is not None:
            ...
            all_neg_ent_feat = (
                self.entity_feat.read(negative_sample.view(-1), "ent_neg_feat") if self.has_feat else None
            )
        if query_structure[0] == "<":  # special handling for inverse query
            ...
            if positive_sample is not None:
                ...
                positive_logits = self.cal_reverse_logit(
                    query_embedding, all_positive_embedding, all_pos_ent_feat, rel_queries
                )
            if negative_sample is not None:
                ...
                negative_logits = self.cal_reverse_logit(
                    query_embedding, all_negative_embedding, all_neg_ent_feat, rel_queries
                )
        ...
        if negative_sample is not None:
            self.sync_read(all_negative_embedding, all_neg_ent_feat)
            if all_negative_embedding is not None:
                all_negative_embedding = self.entity_regularizer(
                    all_negative_embedding.view(batch_size, negative_size, -1)
                )
            if all_neg_ent_feat is not None:
                all_neg_ent_feat = all_neg_ent_feat.view(batch_size, negative_size, -1)
